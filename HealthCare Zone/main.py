...
symptoms_dict = {
    ...
    'brittle_nails': 72, 'swollen_extremities': 73, 'excessive_hunger': 74, 'extra_marital_contacts': 75,
...
def predict():
    if request.method == 'POST':
        ...
        user_symptoms = [s.strip() for s in symptoms.split(',') if s.strip()]  # Strip and remove empty strings
        # Further clean to remove unwanted characters
        user_symptoms = [sym.strip("[]' ") for sym in user_symptoms]
        
        if user_symptoms:  # Ensure that there are valid symptoms before processing
            try:
                ...
                return render_template('index.html', error=f"Invalid symptom: {str(e)}. Please check your input.")
        else:
